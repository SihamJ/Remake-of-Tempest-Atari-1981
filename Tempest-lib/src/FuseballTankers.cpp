...
//constructeur
FuseballTankers::FuseballTankers(const std::string&& name)
{
    this->name = std::move(name);
}

FuseballTankers::FuseballTankers(const std::string&& name, const Color&& c)
    : Enemy(std::move(c))
{
    this->name = std::move(name);
}
...
//constructeur
FuseballTankers::FuseballTankers(const std::string&& name, const  Point&& center, const  Tunel&& h)
...
void FuseballTankers::draw(std::shared_ptr<SDL_Renderer> renderer) {
    ...
    if(render_image(renderer, path, this->init_width, this->init_height, width, height, static_cast<const int>(x),  static_cast<const int>(y), 
                    angle, NULL)){
        std::cout << "Can't load fuseballTanker image " << path <<std::endl;
        return;
    }
}